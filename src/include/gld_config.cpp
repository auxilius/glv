...
void ConfigField::drawText() {
	...
	if (visualizationType == FIELD_TYPE_NONE)
		fontHuge->textOut(border.center.x, border.center.y - fontHuge->getHeight() / 2, FIELD_NAME_NONE, faCenter);
	else if (border.width < 170)  {
		std::string inicial;
		switch (visualizationType) {
			case FIELD_TYPE_TEXTURE:inicial = FIELD_NAME_ABR_TEXTURE; break;
			case FIELD_TYPE_MODEL:	inicial = FIELD_NAME_ABR_MODEL; break;
			case FIELD_TYPE_VALUE:	inicial = FIELD_NAME_ABR_VALUE; break;
		}
		fontHuge->textOut(border.center.x, border.center.y - fontHuge->getHeight() / 2, inicial, faCenter);
	}
	else if (border.width < 250)
		fontLarge->textOut(border.center.x, border.center.y - fontLarge->getHeight() / 2, visualizationTypeCaption[visualizationType], faCenter);
	else 
		fontHuge->textOut(border.center.x, border.center.y - fontHuge->getHeight() / 2, visualizationTypeCaption[visualizationType], faCenter);
};
...
gldConfigurator::gldConfigurator() {
	addingField = false;
	movingField = false;
	resizingField = false;
	selectedField = -1;
...
bool gldConfigurator::addField(int x1, int y1, int x2, int y2) {
	if (abs(x2 - x1) < FIELD_MIN_SIZE_TO_SHOW || abs(y2 - y1) < FIELD_MIN_SIZE_TO_SHOW)
		return false;
